...
class ProductCrud:
    ...
    def by_rating_or_color(cls, rating, color):
        return (Product.objects.filter(rating=rating) | 
            Product.objects.filter(color=color))
    ...
    def below_price_or_above_rating(cls, price, rating):
        return (Product.objects.filter(price_cents__lt=price) |
            Product.objects.filter(rating__gt=rating))
    ...
    def longest_model_name(cls):
        return Product.objects.annotate(length=Length('model')).values('length').aggregate(Max('length'))['length__max']

    @classmethod
    def ordered_by_model_length(cls):
        return Product.objects.annotate(length=Length('model')).order_by('length')